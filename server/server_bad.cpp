...
void *gameServer(void *arg) {
    ...
    //map witch chances left for each   player
    ...
    while (true) {
        ...
        // start when max player count achieved
        if (game->current_players_count == MAX_PLAYER_COUNT) {
            ...
            // send all nicknames
            //std::string nicknames= getNicknamesInGame(game->id);
            // informAllClients(game, nicknames);
            informAllClients(game, "dupa ja\n");
